<template>
  <div>
    <v-container class="page">
      ...
      <div class="nbre">
        <span>Vous avez {{ gettersnbprojetent }} projets ENT</span>.
      </div>
      ...
      <v-container class="tab">
        <v-card-title class="search_and_add">
          ...
          >
            <Icon icon="carbon:add-alt" /> Ajouter un projets ent
          </v-btn>
        </v-card-title>
        ...
        <v-dialog v-model="dialogEdit" max-width="500px">
          <v-card class="card">
            ...
            <v-form ref="form" v-model="valid">
              ...
              <v-card-actions v-if="dialog_title == 'Modifier le projet ent'">
                <v-spacer></v-spacer>
                <v-btn
                  :disabled="!valid"
                  color="blue darken-1"
          ...
          <v-card class="card">
            ...
            <v-card-actions>
              ...
              <v-btn
                v-if="
                  dialog_confirmation ==
                  'Etes-vous sûr de vouloir modifier le projet ent ?'
                "
                color="blue darken-1"
                text
...
export default {
  ...
  data() {
    return {
      valid: false,
      inactiveprojecttext: "Afficher les projets ent inactifs",
  ...
  computed: {
    gettersprojetent() {
      return this.$store.getters.projetent; // Provide us the new state of datas
    },
    gettersnbprojetent() {
      return this.$store.getters.nbprojetent;
    },
  },
  ...
  methods: {
    ...
    async edit_projet_ent() {
      ...
      if (this.editedItem.id !== undefined) {
        ...
        try {
          ...
          this.$store.dispatch("getProjetEnt"); // to reload the page and see the modifications
          this.liste_index++;
          this.dialog_text =
            "Le projet ent" + this.editedItem.libelle_long + " a été modifié.";
          ...
          this.dialog_text =
            ...
            " n'a pas été modifié car il manque des données.";
        ...
        this.dialogEdit = false;
        this.editedItem.id_type_projet_territorial = 1;

        try {
          const response = await axios.post(path_add, this.editedItem);
          console.log(response);
          this.$store.dispatch("getProjetEnt");
          this.liste_index++;
          this.dialog_text =
            "Le projet ent" + this.editedItem.libelle_long + " a été ajouté.";
    ...
    title_edit() {
      this.dialog_title = "Modifier le projet ent";
    },
    title_add() {
      this.dialog_title = "Nouvelle projet ent";
    },
    edit_dialog_confirmation() {
    ...
    add_dialog_confirmation() {
      this.dialog_confirmation =
        "Etes-vous sûr de vouloir ajouter le projet ent ?";
    },
    inactiveprojectfunc() {
      if (!this.inactiveproject) {
        this.inactiveprojecttext = "Afficher le projet ent inactifs";
      } else {
        this.inactiveprojecttext = "Cacher le projet ent inactifs";
      }
    },
  ...
  mounted() {
    // We do the API callS
    this.$store.dispatch("getProjetEnt");
    this.$store.dispatch("getNbProjetEnt");
